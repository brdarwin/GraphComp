...
void displayPersp(void) {
  ...
  glPushMatrix();
  {
    ...
    glBindTexture(GL_TEXTURE_2D, idTextureTampo);
    glBegin(GL_QUADS);{
    
    glTexCoord2f(0.0f, 0.0f);
    glVertex3f( -1.1, 0.33, 0.6);
...
void init() {
  ...
  table.textureId = loadTGA("wood.tga");
  loadObj("table.obj", table);

  ball.textureId = loadTGA("ball.tga");
  loadObj("ball.obj", ball);

  idTextureTampo = loadTGA("table.tga");
  loadObj("table.obj", table);
